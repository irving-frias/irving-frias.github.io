...
document.addEventListener('DOMContentLoaded', function() {
  let codeStepperCopy = document.querySelectorAll('.code-stepper-copy');

  if (codeStepperCopy) {
    [...codeStepperCopy].forEach((element) => {
      element.addEventListener('click', function(e) {
        let target = e.target;
        target = target.closest('.code-stepper-copy');

        let data = target.dataset.title;
        copyContent(data);
      });
    });
  }
